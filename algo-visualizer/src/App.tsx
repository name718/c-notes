...
function App() {
  ...
  const currentSnapshot = useMemo(() => 
    steps[currentStepIndex] || { data: [], pointers: {}, activeLines: [], description: 'Ready' },
    ...
    <div className="flex h-screen w-full bg-background text-white font-sans overflow-hidden select-none">
      ...
      <aside className="w-12 flex flex-col items-center py-4 bg-background border-r border-border space-y-6">
        ...
        <div className="flex flex-col space-y-6 text-gray-500">
          <Database size={20} className="hover:text-accent cursor-pointer transition-colors" />
          <Settings size={20} className="hover:text-accent cursor-pointer transition-colors" />
          <HelpCircle size={20} className="hover:text-accent cursor-pointer transition-colors" />
        </div>
      ...
      <aside className="w-60 bg-sidebar border-r border-border hidden md:flex flex-col">
        <div className="p-4 border-b border-border">
          <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Explorer</h2>
        </div>
        <nav className="flex-1 p-2 space-y-1 overflow-y-auto">
          <div className="text-[11px] font-bold text-gray-600 px-3 py-2 uppercase tracking-wider">Sorting Algorithms</div>
      ...
      <main className="flex-1 flex flex-col relative min-w-0 bg-[#0A0A0A]">
        <header className="h-10 border-b border-border flex items-center px-4 bg-background/50 backdrop-blur-sm">
          <span className="text-[10px] text-gray-500 uppercase tracking-wider">Sorting</span>
          <span className="mx-2 text-gray-700">/</span>
          <span className="text-[10px] text-accent uppercase tracking-wider font-bold">Selection Sort</span>
        </header>
        ...
        <div className="absolute bottom-10 left-1/2 -translate-x-1/2 bg-[#1A1A1A]/90 backdrop-blur-xl border border-white/10 px-6 py-2 rounded-full flex items-center gap-6 shadow-2xl">
          <div className="flex items-center gap-1">
            <button onClick={() => goToStep(0)} title="Reset" className="p-2 text-gray-400 hover:text-accent transition-colors"><RotateCcw size={16} /></button>
            <button onClick={prevStep} title="Prev Step" className="p-2 text-gray-400 hover:text-accent transition-colors"><ChevronLeft size={20} /></button>
            <button 
              onClick={togglePlay}
            ...
            <button onClick={nextStep} title="Next Step" className="p-2 text-gray-400 hover:text-accent transition-colors"><ChevronRight size={20} /></button>
          </div>
      ...
      <aside className="w-72 bg-sidebar border-l border-border hidden xl:flex flex-col">
        <div className="p-4 border-b border-border flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">Source</h2>
          </div>
